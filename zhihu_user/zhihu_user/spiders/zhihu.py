...
class ZhihuSpider(scrapy.Spider):
    ...
    #定义一个start_user存储我们找的大V账号
    start_user = "excited-vczh"

    #user_url存储查询用户信息的url
    user_url = "https://www.zhihu.com/api/v4/members/{user}?include={include}"
    #这里把查询的参数单独存储为user_query
    ...
    #follows_url存储的是关注列表的url，fllows_query存储的为查询参数。这里涉及到offset和limit参数，它们是关于翻页的，0,20表示第一页
    ...
    #followers_url是获取粉丝的url地址，followers_query存储的为查询参数
    ...
    def parse_user(self,response):
        ...
        #这里在返回item的同时返回request请求，继续递归使用关注用户信息的用户获取他们的关注列表
        yield item
    ...
    def parse_follows(self,response):
        '''
        用户关注列表的解析，这里返回的是json数据，这里有两个字段data和page，其中page是分页信息
        :param response:
        :return:
        '''
        ...
        #这里判断page是否存在并且判断page里的参数is_end判断是否为False,如果为False表示不是最后一页，否则是最后一页
